...
import { pdfFormsService, documentStore } from "@/services/documentManagementService";
...
export default function PDFFormsPage() {
  ...
  const handleSaveDoc = async (id: string | number, updates: Partial<DocumentItem>) => {
    await documentStore.updatePDFForm(id, updates);
  ...
  return (
    <>
      <ModuleRenderer
        config={pdfFormsModuleConfig}
        service={pdfFormsService}
      ...
      {selectedDoc && (
        <PDFViewer
          ...
          viewerOptions={pdfFormsModuleConfig.viewerOptions}
          formOptions={pdfFormsModuleConfig.formOptions}
